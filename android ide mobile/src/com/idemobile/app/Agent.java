...
public class Agent {
     ...
     *   • успешный read-only повтор (fs_read/fs_search/…) — НЕ обрываем, а мягко
     *     подталкиваем перейти к действию (как в web); общий предохранитель — maxTool;
    ...
    static class ToolLoopDetector {
        ...
        LoopSignal observe(String tool, JSONObject args, boolean ok, String error) {
            ...
            // 1a) Успешный read-only повтор — подталкиваем РАНЬШЕ (со 2-го повтора),
            // а после 5 одинаковых read-only повторов подряд — ЖЁСТКО обрываем.
            // Раньше read-only не останавливался никогда: слабая модель могла
            // перечитывать один и тот же файл десятки раз (раздувая контекст до
            // 403-лимита шлюза) — теперь этот сценарий пресекается.
            if (readonly && ok) {
                if (sameCallCount >= 5) {
                    return LoopSignal.stop("Агент " + sameCallCount + " раз подряд вызывает `" + tool
                            + "` с одинаковыми параметрами (перечитывает одно и то же) — застрял в цикле. "
                            + "Прерываю, чтобы не раздувать контекст и не упереться в лимит провайдера.");
                }
                if (sameCallCount >= 2 && roNudges < 4) {
                    roNudges++;
    ...
    private String systemPrompt(JSONObject provider) {
        ...
        sb.append("ЯЗЫК ОБЩЕНИЯ: используй язык сообщений пользователя. Код, имена файлов не переводи.\n\n");
        ...
        if (hasUserSys)
            sb.append("\n\n━━━ НАПОМИНАНИЕ: ПРАВИЛА ПОЛЬЗОВАТЕЛЯ (ВЫСШИЙ ПРИОРИТЕТ, ПЕРЕОПРЕДЕЛЯЮТ ВСЁ ВЫШЕ) ━━━\n")
              .append("Соблюдай эти правила строго на протяжении ВСЕГО диалога, не только в первом ответе:\n")
              .append(userSys.trim());
        return sb.toString();
    }
