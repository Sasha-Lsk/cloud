    ...
    static JSONArray normalizeMessages(JSONArray messages, boolean useTools) throws JSONException {
        ...
        for (int i = 0; i < messages.length(); i++) {
            ...
            if (useTools && "assistant".equals(role) && m.optJSONArray("_toolCalls") != null
                ...
                // ВАЖНО: Mistral отвергает assistant, у которого одновременно есть content
                // И tool_calls ("must have either content or tool_calls, but not both",
                // code 3240). Поэтому у tool_calls-сообщения content ВСЕГДА null —
                // сопутствующий reasoning модели в native-режиме не критичен.
        ...
        try {
            for (int i = 0; i < out.length(); i++) {
                ...
                boolean mergeable = sameRole
                        ...
                        && !mTools && !aTools
                        && (anchor.opt("content") instanceof String) && (m.opt("content") instanceof String);
                if (mergeable) {
                    ...
                    String a = anchor.optString("content", "");
                    String b = m.optString("content", "");
                    String joined = (a + (b.isEmpty() ? "" : "\n\n" + b)).trim();
                    anchor.put("content", joined.isEmpty() ? "(продолжаю)" : joined);
                    for (JSONObject n : notes) mergedArr.put(n);
                } else if (sameRole && "assistant".equals(role) && mTools && !aTools
                        && (anchor.opt("content") instanceof String)) {
        ...
        //   {"message":"Expected last role User or Tool (or Assistant with prefix True)
        //    for serving but got assistant", "code":"3230"}
        // Такой хвост-assistant/hвост-system возникает штатно: напр. после turn'а, где
        // модель дала assistant-текст без валидного tool-вызова, а следом в buildLLMMessages
        // добавилась system-заметка (правила пользователя / nudge). Другие провайдеры это
        // прощают, Mistral — нет, и задача падала «агент ничего не делает». Дописываем
        // безопасное user-продолжение, если хвост не user/tool. НЕ трогаем tool_calls-хвост
        // (assistant с tool_calls допустим — за ним должны идти tool-ответы, что уже
        // гарантирует fillMissingToolResults на стороне Agent).
        if (out.length() > 0) {
            JSONObject last = out.optJSONObject(out.length() - 1);
            if (last != null) {
                String lr = last.optString("role");
                boolean asstWithTools = "assistant".equals(lr)
                        && last.optJSONArray("tool_calls") != null
                        && last.optJSONArray("tool_calls").length() > 0;
