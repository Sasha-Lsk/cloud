...
const App = (() => {
  ...
  const ABOUT_COPYRIGHT = '© 2026 by Claude Opus 4.8';

  // Окно «О приложении». Внизу — красная кнопка полного (жёсткого) сброса.
  function showAbout() {
    const body = U.el('div', { style:{ fontSize:'13px', lineHeight:'1.65' } });
    body.innerHTML =
    ...
    }, 'Сброс всех настроек клиента');
    resetBtn.addEventListener('click', () => hardResetAll());
    body.appendChild(resetBtn);
    UI.dialog({ title:'О приложении', body });
  }

  // Жёсткий полный сброс клиента: стираем ВСЁ локальное состояние без остатков —
  // localStorage целиком, все IndexedDB-базы приложения (виртуальная ФС, импорт-ФС,
  // сохранённые handle локальных папок), Cache Storage и sessionStorage. После этого
  // перезагружаем страницу, чтобы приложение стартовало с чистого листа.
  async function hardResetAll() {
    const ok = await UI.confirm(
      'Полностью сбросить ВСЕ настройки и данные клиента? Будут удалены: провайдеры и ключи, история чатов, авторизация GitHub, все файлы виртуальной ФС и импортированные папки. Действие необратимо.',
      { title:'Сброс всех настроек', okText:'Сбросить всё', danger:true });
    if (!ok) return;
    try {
      // 1) localStorage + sessionStorage — все ключи приложения.
      try { localStorage.clear(); } catch(_){}
      try { sessionStorage.clear(); } catch(_){}
      // 2) IndexedDB — удаляем все известные базы приложения. Если браузер
      //    поддерживает databases() — добьём и остальные, чтобы не осталось хвостов.
      const known = ['mide.config.v1', 'mide-fs', 'mide-import-fs', 'mide-localfs-handles'];
      ...
      try {
        if (indexedDB.databases) {
          ...
          for (const d of (list || [])) if (d && d.name && !names.includes(d.name)) names.push(d.name);
        }
      } catch(_){}
      await Promise.all(names.map(delDb));
      // 3) Cache Storage (на случай Service Worker / offline-кэша).
      try {
    ...
    // Перезагружаем начисто.
    try { location.reload(); } catch(_) { location.href = location.href; }
  }

  return { switchTab, importFiles, exportZip, githubLogin, githubLoginDeviceFlow, githubLoginBrowserPAT, githubLoginManualPAT, githubClone, updateGithubHint, boot, showAbout, hardResetAll };
})();
window.App = App;
