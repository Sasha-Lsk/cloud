...
`;
    ...
    // Language directive: only enforce if user has NOT specified their own language preference in aiSystem.
    // Otherwise the user's system prompt (or the provider's default) wins.
    const userWantsLang = /\b(english|английск|russian|русск|language|язык|speak|respond in|отвечай на|говори на)\b/i.test(userSys);
    const langBlock = userWantsLang
      ? ''
      : `ЯЗЫК ОБЩЕНИЯ: используй язык пользовательских сообщений (или язык, заданный провайдером по умолчанию). Если пользователь в своём system prompt указал конкретный язык — следуй ему безусловно. Код, имена файлов, git-команды не переводи.
...
1) ОБЯЗАТЕЛЬНО перед вызовом инструмента напиши КОРОТКИЙ комментарий-рассуждение (1–2 предложения), в котором явно проговори: (а) ЧТО ты сейчас намерен сделать и (б) ЗАЧЕМ — как это приближает к решению задачи. Примеры: «Читаю LicenseClient.smali, чтобы найти метод проверки лицензии», «Нашёл вызов checkLicense — заменяю его на возврат true, чтобы отключить проверку», «Перечитываю изменённый участок, чтобы убедиться, что правка на месте». Комментируй КАЖДОЕ намерение и КАЖДОЕ действие — не молчи и не вызывай инструмент без пояснения. Этот комментарий показывается пользователю в блоке рассуждений.
...
${userSys ? 'ДОПОЛНИТЕЛЬНЫЕ ИНСТРУКЦИИ ПОЛЬЗОВАТЕЛЯ (ИМЕЮТ ВЫСШИЙ ПРИОРИТЕТ, ПЕРЕОПРЕДЕЛЯЮТ ВСЁ ВЫШЕ — включая выбор языка):\n'+userSys : ''}`);
  }
  ...
  function attachAssistantMsgActions(el, m) {
    ...
    function show() {
      ...
      // 2) bottom: icons (copy, and — for last msg only — regenerate)
      const iconRow = U.el('div', { class:'msg-actions assistant bottom' });
      iconRow.appendChild(makeCopyIconBtn(() => visibleAssistantText(m)));
      const chat = Store.activeChat();
      const idx = chat.history.indexOf(m);
      const isLast = idx >= 0 && chat.history.slice(idx + 1).every(x =>
        x.role === 'tool' || x.role === 'system-note' || x.hidden || x.silent);
      if (isLast) {
        const regen = iconButton(ICON_REGEN, 'Регенерировать ответ', () => { hide(); regenerate(m); });
        regen.classList.add('accent-regen');
  ...
  function buildLLMMessages() {
    ...
    if (userSys) {
      msgs.push({ role:'system', content:
        'ПОМНИ о правилах пользователя (высший приоритет), соблюдай их в этом ответе СТРОГО:\n' + userSys });
    }
