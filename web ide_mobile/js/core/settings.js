...
const Settings = (() => {
  ...
  function open() {
    ...
    // Блок авторизации GitHub из «Настроек ИИ» убран намеренно: авторизация и
    // управление GitHub полностью доступны на вкладке «GitHub». Дублировать её
    // здесь не нужно. (renderGithubSection оставлена в коде, но не вызывается.)
  ...
  function renderOptionsSection() {
    ...
    const optIO = U.el('div', { class:'field' });
    const ioRow = U.el('div', { style:{display:'flex', gap:'8px', flexWrap:'wrap'} });
    const exportBtn = U.el('button', { class:'btn small' }, '⤓ Экспорт настроек ИИ');
    const importBtn = U.el('button', { class:'btn small' }, '⤒ Импорт настроек ИИ');
    ioRow.appendChild(exportBtn); ioRow.appendChild(importBtn);
    optIO.appendChild(U.el('label', {}, 'Экспорт / импорт настроек ИИ'));
    optIO.appendChild(ioRow);
    exportBtn.addEventListener('click', (e) => { e.preventDefault(); exportAiSettings(); });
    importBtn.addEventListener('click', (e) => { e.preventDefault(); importAiSettings(); });
    optIO.appendChild(spoiler('Экспорт / импорт настроек ИИ',
      'Сохраняет в файл (или загружает из файла) ВСЕ настройки ИИ вместе с созданными профилями провайдеров (Base URL, ключи, модели, CORS-прокси, доп. заголовки), дополнительный system prompt и параметры поведения (лимиты, таймаут, throttle, автоповтор). Чаты, файлы и авторизация GitHub НЕ входят в этот экспорт. ВНИМАНИЕ: импорт ПОЛНОСТЬЮ заменяет текущие настройки ИИ и профили импортированными (существующие удаляются).'));
    ...
    sect.appendChild(optIO);

    return sect;
  }

  // Что именно входит в «настройки ИИ»: профили провайдеров + активный профиль +
  // доп. system prompt + все параметры поведения ИИ. GitHub/чаты/файлы НЕ входят.
  function collectAiSettings() {
    const s = Store.get();
    return {
      _type: 'mobile-ide.ai-settings',
      _version: 1,
  ...
  async function exportAiSettings() {
    try {
      const data = collectAiSettings();
      const json = JSON.stringify(data, null, 2);
      const blob = new Blob([json], { type:'application/json' });
      const name = 'mobile-ide-ai-settings.json';
      if (window.Explorer && Explorer.saveBlob) await Explorer.saveBlob(blob, name, { preferShare:true });
      else {
      ...
      UI.toast('Настройки ИИ экспортированы: ' + name, 'ok');
    } catch (e) {
      UI.toast('Не удалось экспортировать: ' + (e.message || e), 'err');
    }
  }

  // Импорт НАЧИСТО: существующие профили и настройки ИИ ПОЛНОСТЬЮ заменяются
  // импортированными. Чаты, файлы и GitHub не трогаем.
  function importAiSettings() {
    ...
    inp.addEventListener('change', async () => {
      ...
      try {
        ...
        UI.toast('Файл не является корректным JSON', 'err');
        return;
      }
      if (!data || typeof data !== 'object' || !Array.isArray(data.providers)) {
        UI.toast('Не похоже на файл настроек ИИ (нет списка providers)', 'err');
        return;
      }
      const ok = await UI.confirm(
        'Импортировать настройки ИИ? Текущие профили провайдеров и настройки ИИ будут ПОЛНОСТЬЮ ЗАМЕНЕНЫ импортированными (' + data.providers.length + ' профил(ь/я/ей)). Чаты и файлы не затрагиваются.',
        { title:'Импорт настроек ИИ', okText:'Заменить', danger:true });
      if (!ok) return;
      try {
        const st = Store.get();
        // Полная замена профилей и настроек ИИ.
        st.providers = data.providers;
        st.activeProviderId = data.activeProviderId || (data.providers[0] ? data.providers[0].id : null);
        st.aiSystem = typeof data.aiSystem === 'string' ? data.aiSystem : '';
        st.settings = (data.settings && typeof data.settings === 'object') ? data.settings : {};
        // Чтобы seed OPENROUTER не «воскрес» после импорта пустого списка.
        st._seededOpenRouter = true;
        Store.save();
        try { AI.updateHeader(); } catch(_){}
        refresh();
        UI.toast('Настройки ИИ импортированы (заменено начисто)', 'ok');
      } catch (e) {
        UI.toast('Не удалось импортировать: ' + (e.message || e), 'err');
      }
    });
    inp.click();
  }
