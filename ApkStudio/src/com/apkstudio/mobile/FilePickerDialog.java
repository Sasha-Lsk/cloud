...
public class FilePickerDialog {
    ...
    private void refresh() {
        ...
        adapter.clear();
        // ".." вверх
        if (current.getParentFile() != null) { entries.add(null); adapter.add("[..] вверх"); }
        ...
        for (File d : dirs) { entries.add(d); adapter.add("[dir] " + d.getName()); }
        if (mode == MODE_APK)
            for (File a : apks) { entries.add(a); adapter.add("[APK] " + a.getName()); }
